...
class LobsterFile(MSONable):
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        if "spins" in dictionary:
            dictionary["spins"] = [
                s.value if isinstance(s, Spin) else s for s in dictionary["spins"]
            ]
    ...
    def from_dict(cls, d: dict[str, Any]) -> Self:
        ...
        if instance.spins:
            instance.spins = [
                Spin(s) if isinstance(s, int) else s for s in instance.spins
            ]
...
class LobsterInteractionsHolder(LobsterFile):
    ...
    def get_label_from_interaction(
        ...
        if latex_format:
            ...
            label = re.sub(r"_([^\[\]()\s]+)", r"_{\1}", label)
            label = re.sub(
                r"\[\s*(-?\d+)\s+(-?\d+)\s+(-?\d+)\s*\]", r"[$\1 \; \2 \; \3$]", label
            )
